...
65. 查看 [Steam 硬件调查]()中的 Primary Display Resolution 这一部分。最流行的，使用超过半数的分辨率是`1920x1080`，我们这款游戏基础分辨率可以整数倍放大至`1920x1080`。第二受欢迎的分辨率是`1366x768`，`480x270`不能整数倍放大至该分辨率。思考，当游戏全屏显示在玩家屏幕上时，哪些选项可以用来处理这些独特的分辨率？
